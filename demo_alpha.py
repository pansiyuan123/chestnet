...
from chestx.loss import MultiweightBCELoss
...
def main_chest():
    ...
    net = densenet121_chest(num_classes=args.num_classes, kmax=5, num_maps=12).to(device)
    ...
    # net.load_state_dict(torch.load('./chestx/logs_weight2/model_5.pth'))

    # Training
    AUROCw = [.5] * args.num_classes
    print("Staring Training ...")
    for epoch in range(1, args.epochs + 1):
        scheduler.step()
        AUROCw = torch.FloatTensor(AUROCw).to(device)
        criterion = MultiweightBCELoss(AUROCw)
        ...
        print("Waiting for Testing Train...")
        ...
        with torch.no_grad():
            ...
            AUROCw = compute_AUCs(gt_npy, pred_npy)
            AUROC_avg = np.array(AUROCw).mean()
            print('The Train average AUROC is {AUROC_avg:.4f}'.format(AUROC_avg=AUROC_avg))
            print('AUROC weight cost time : {:.2f}'.format(time.time() - state['test_time']))
        state['test_time'] = time.time()
