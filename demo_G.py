...
from chestx.loss import GHMC_Loss, GHMS_Loss
...
def main_chest():
    ...
    criterion = GHMS_Loss(device, 25, 0.75)
    criterion = criterion.to(device)
    optimizer = torch.optim.SGD(net.parameters(),
                                lr=args.lr,
                                momentum=args.momentum,
                                weight_decay=args.weight_decay)
    scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[12, 15, 18], gamma=0.1)
